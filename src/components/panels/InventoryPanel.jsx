...
const InventoryPanel = (
    ...
    // Lot Details Memo moved to top level
    const lotDetails = React.useMemo(() => {
        if (!selectedLot) return null;
        const lotLots = (masterData.cuttingStock || []).filter(l => l.lotNo === selectedLot);
        const cutB = lotLots.reduce((s, l) => s + (l.borka || 0), 0);
        const cutH = lotLots.reduce((s, l) => s + (l.hijab || 0), 0);

        const prods = (masterData.productions || []).filter(p => p.lotNo === selectedLot && p.status === "Received");
        const readyB = prods.reduce((s, p) => s + (p.receivedBorka || 0), 0);
        const readyH = prods.reduce((s, p) => s + (p.receivedHijab || 0), 0);
        const nostoB = prods.reduce((s, p) => s + (p.nostoBorka || 0), 0);
        const nostoH = prods.reduce((s, p) => s + (p.nostoHijab || 0), 0);

        const delivered = (masterData.deliveries || []).filter(d => d.lotNo === selectedLot);
        const delB = delivered.reduce((s, d) => s + (d.qtyBorka || 0), 0);
        const delH = delivered.reduce((s, d) => s + (d.qtyHijab || 0), 0);

        return {
            design: lotLots[0]?.design,
            color: lotLots[0]?.color,
            size: lotLots[0]?.size,
            cut: cutB + cutH,
            ready: readyB + readyH,
            nosto: nostoB + nostoH,
            delivered: delB + delH,
            stock: (readyB + readyH) - (delB + delH),
            readyB, readyH, delB, delH
        };
    }, [selectedLot, masterData]);
    ...
    const handleDelivery = (e) => {
        ...
        setMasterData(prev => {
            // Stock Validation
            if (lotNo) {
                const lotLots = (prev.cuttingStock || []).filter(l => l.lotNo === lotNo);
                const prods = (prev.productions || []).filter(p => p.lotNo === lotNo && p.status === "Received");
                const ready = prods.reduce((s, p) => s + (p.receivedBorka || 0) + (p.receivedHijab || 0), 0);
                const delivered = (prev.deliveries || []).filter(d => d.lotNo === lotNo);
                const del = delivered.reduce((s, d) => s + (d.qtyBorka || 0) + (d.qtyHijab || 0), 0);

                if ((qtyB + qtyH) > (ready - del)) {
                    showNotify(`লট #${lotNo}-এ পর্যাপ্ত স্টক নেই! (উপলব্ধ: ${ready - del} পিস)`, "error");
                    return prev;
                }
            }
                                ...
                                <form onSubmit={handleDelivery} className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest ml-1">লট নম্বর (Select Lot)</label>
                                            <select
                                                name="lotNo"
                                                className="premium-input !h-12 uppercase font-bold bg-white dark:bg-slate-900"
                                                value={selectedLot}
                                                onChange={(e) => setSelectedLot(e.target.value)}
                                            >
                                                <option value="">লট নির্বাচন করুন (Optional)</option>
                                                {[...new Set((masterData.cuttingStock || []).map(l => l.lotNo))].sort((a, b) => b - a).map(l => (
                                                    <option key={l} value={l}>LOT #{l}</option>
                                                ))}
                                            </select>
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-bold text-black dark:text-white dark:text-white uppercase tracking-widest ml-1">ক্লায়েন্টের নাম (Client / Party)</label>
                                            <div className="relative group">
                                                <Users className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                                <select name="receiver" className="premium-input !pl-12 !h-12 text-sm uppercase font-bold bg-white dark:bg-slate-900" required>
                                    ...
                                    {lotDetails && (
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-950 p-6 rounded-2xl text-white shadow-2xl relative overflow-hidden group mb-6">
                                            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><Activity size={80} /></div>
                                            <div className="relative z-10">
                                                <div className="flex justify-between items-start mb-6">
                                                    <div>
                                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-400 mb-1">Lot Intelligence</p>
                                                        <h4 className="text-2xl font-black italic tracking-tighter uppercase">{lotDetails.design} // {lotDetails.color}</h4>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-[9px] font-bold uppercase opacity-40">Ready Stock</p>
                                                        <p className="text-2xl font-black italic text-emerald-400">{lotDetails.stock} PCS</p>
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-4 gap-4 border-t border-white/10 pt-4">
                                                    <div><p className="text-[8px] font-bold opacity-40 uppercase">Cutting</p><p className="text-lg font-bold">{lotDetails.cut}</p></div>
                                                    <div><p className="text-[8px] font-bold opacity-40 uppercase">Ready</p><p className="text-lg font-bold text-emerald-400">{lotDetails.ready}</p></div>
                                                    <div><p className="text-[8px] font-bold opacity-40 uppercase text-rose-400">Nosto</p><p className="text-lg font-bold text-rose-400">{lotDetails.nosto}</p></div>
                                                    <div><p className="text-[8px] font-bold opacity-40 uppercase text-blue-400">Sent</p><p className="text-lg font-bold text-blue-400">{lotDetails.delivered}</p></div>
                                                </div>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-1.5">
                                            ...
                                            <select
                                                ...
                                                defaultValue={lotDetails?.design || ""}
                                                key={`design-${selectedLot}`}
                                                required
                                            >
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <select
                                                name="color"
                                                className="premium-input !h-12"
                                                defaultValue={lotDetails?.color || ""}
                                                key={`color-${selectedLot}`}
                                            >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="space-y-1.5">
                                            ...
                                            <select
                                                name="size"
                                                className="premium-input !h-12 text-sm text-center"
                                                defaultValue={lotDetails?.size || ""}
                                                key={`size-${selectedLot}`}
                                            >
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <input name="qtyBorka" type="number" className="premium-input !h-12 text-xl font-bold text-center !bg-slate-100 dark:!bg-slate-800/50" placeholder="0" defaultValue={lotDetails?.stock > 0 ? lotDetails.readyB - lotDetails.delB : 0} key={`b-${selectedLot}`} />
                                        </div>
                                        <div className="space-y-1.5">
                                            ...
                                            <input name="qtyHijab" type="number" className="premium-input !h-12 text-xl font-bold text-center !bg-slate-100 dark:!bg-slate-800/50" placeholder="0" defaultValue={lotDetails?.stock > 0 ? lotDetails.readyH - lotDetails.delH : 0} key={`h-${selectedLot}`} />
                                        </div>
                                    </div>
                                    <div className="pt-6">
                                    ...
                                    {(masterData.deliveries || []).slice(0, 10).map((d, i) => (
                                        <div key={i} className="p-5 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center group hover:border-slate-300 dark:hover:border-slate-600 transition-all">
                                            <div>
                                                ...
                                                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{d.receiver} • {d.date} {d.lotNo && `• Lot #${d.lotNo}`}</p>
                                            </div>
                                            <div className="text-right">
